...
const ACCESS = {
  ...
  adminHash: "8de49a49960c40141c432ab83264a4b195927445452af34a263268454b07d08e",
};
...
function onListClick(e) {
  ...
  if (action === "copy") {
    copyText(entry.key).then((ok) =>
      toast(ok ? "API key copied!" : "Copy failed", !ok),
    );
...
function setMode(mode) {
  ...
  els.switchMode.textContent = login
...
async function tryLogin() {
  ...
  if (
    user.toLowerCase() === ACCESS.username.toLowerCase() &&
    hash === ACCESS.adminHash
  ) {
...
async function tryBackupCode() {
  ...
  localStorage.setItem(
    "api_key_vault_codes",
    JSON.stringify(BACKUP_CODE_HASHES),
  );
...
function init() {
  ...
  els.toggleAdminPass.addEventListener("click", () => {
    els.adminPass.type =
      els.adminPass.type === "password" ? "text" : "password";
  });
